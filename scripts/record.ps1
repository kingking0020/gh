...
try {
    ...
    $chromePids = @($chromeProcesses | Select-Object -ExpandProperty Id)
    ...
    [WindowAPI]::EnumWindows({
        ...
        try {
            ...
            [WindowAPI]::GetWindowText($hWnd, $titleBuilder, 512) | Out-Null
            $title = $titleBuilder.ToString()

            if ([string]::IsNullOrWhiteSpace($title)) { return $true }

            [uint32]$windowProcessId = 0
            [WindowAPI]::GetWindowThreadProcessId($hWnd, [ref]$windowProcessId) | Out-Null

            if ($chromePids -contains [int]$windowProcessId) { return $true }

            Log "Minimizing window: $title"
            [WindowAPI]::ShowWindow($hWnd, [WindowAPI]::SW_MINIMIZE) | Out-Null
        }
        catch {
            Log "Window enumeration error: $($_.Exception.Message)"
        }
    ...
    [WindowAPI]::ShowWindow($chromeWindow, [WindowAPI]::SW_MAXIMIZE) | Out-Null
    Start-Sleep -Milliseconds 500
    [WindowAPI]::SetForegroundWindow($chromeWindow) | Out-Null
    Start-Sleep -Milliseconds 500

    Log "Creating red click marker."

    $redDot = New-Object System.Windows.Forms.Form
    $redDot.FormBorderStyle = [System.Windows.Forms.FormBorderStyle]::None
    $redDot.StartPosition = [System.Windows.Forms.FormStartPosition]::Manual
    $redDot.Location = New-Object System.Drawing.Point(79, 609)
    $redDot.Size = New-Object System.Drawing.Size(10, 10)
    $redDot.BackColor = [System.Drawing.Color]::Red
    $redDot.TopMost = $true
    ...
    Remove-Item $videoPath, $screenshotPath, $ffmpegOut, $ffmpegErr -Force -ErrorAction SilentlyContinue
    ...
    $ffmpegProcess = Start-Process -FilePath "ffmpeg.exe" -ArgumentList $ffmpegArgs -RedirectStandardOutput $ffmpegOut -RedirectStandardError $ffmpegErr -PassThru -WindowStyle Hidden
    ...
    $screenshotInfo = Get-Item $screenshotPath
    Log "Screenshot size: $($screenshotInfo.Length) bytes"

    Log "Running ffprobe."

    $ffprobeOutput = & ffprobe -v error -show_entries format=duration,size -of default=noprint_wrappers=1 $videoPath 2>&1
    Add-Content -Path $logFile -Value ($ffprobeOutput -join [Environment]::NewLine)
